...
signal int_xen_reg         : std_logic;
signal int_xen             : std_logic;
...
type state_type is (idle, smpls_present, xen_detect, rd_samples, wait_rd_cycles);
signal current_state, next_state : state_type;

--inst0
signal inst0_pulse_div     : std_logic;
--inst1
signal inst1_pulse_div     : std_logic;
...
-- ----------------------------------------------------------------------------
diq_smpl_0 <= fifo_q(4*iq_width-1 downto 3*iq_width);
diq_smpl_1 <= fifo_q(3*iq_width-1 downto 2*iq_width);
diq_smpl_2 <= fifo_q(2*iq_width-1 downto 1*iq_width);
diq_smpl_3 <= fifo_q(1*iq_width-1 downto 0*iq_width);

-- ----------------------------------------------------------------------------
--Internal mode selection for DIQ position
-- "00" - MIMO DDR both channels enabled, SISO DDR, TXIQ_PULSE
-- "01" - MIMO DDR first channel enabled
-- "10" - MIMO DDR second channel enabled
-- "11" - SISO SDR 
-- ----------------------------------------------------------------------------
 int_mode <= "00" when (mimo_en_sync='0' AND ddr_en_sync='1') OR (mimo_en_sync='1' AND ch_en_sync="11") OR trxiqpulse_sync ='1' else 
             "01" when (mimo_en_sync='1' AND ch_en_sync="01") else
             "10" when (mimo_en_sync='1' AND ch_en_sync="10") else
             "11";
             
-- ----------------------------------------------------------------------------
--Muxes for fsync signal
-- ----------------------------------------------------------------------------  
mux_fsync_H <= "1111" when  (mimo_en_sync ='0' AND ddr_en_sync ='1' AND trxiqpulse_sync='0') else 
               "0101";

mux_fsync_L <= "0000" when  (mimo_en_sync ='0' AND ddr_en_sync = '1') OR trxiqpulse_sync='1'  else 
               "0101";
               
int_fsync_L <= (not mux_fsync_L(3) & not mux_fsync_L(2) & not mux_fsync_L(1) & not mux_fsync_L(0)) when fidm_sync = '0' else 
               mux_fsync_L;
               
int_fsync_H <= (not mux_fsync_H(3) & not mux_fsync_H(2) & not mux_fsync_H(1) & not mux_fsync_H(0)) when fidm_sync = '0' else 
               mux_fsync_H;
...
mux_pos0_L <=  diq_smpl_0     when int_mode = "11" else 
               ...
               diq_smpl_3    when (int_mode = "01" AND mux_sig = '0') else
               diq_smpl_1;
                  
mux_pos1_L <=  diq_smpl_3     when int_mode = "00" else 
               (others=>'0')  when int_mode = "01" else
               diq_smpl_3    when (int_mode = "10" AND mux_sig = '0') else
               diq_smpl_1;
               
mux_pos2_L <=  (others=>'0')  when int_mode(0) = '0' else
               diq_smpl_1    when (int_mode = "01" AND mux_sig = '1') else
               diq_smpl_3     when int_mode = "01" else 
               diq_smpl_2;               
                  
mux_pos3_L <=  (others=>'0')  when int_mode(1) = '0' else
               diq_smpl_1    when (int_mode = "10" AND mux_sig = '1') else
               diq_smpl_3; 
...
--state machine combo
-- ----------------------------------------------------------------------------
fsm : process(current_state, fifo_rdempty, rd_wait_cnt, rd_wait_cnt_max_reg, int_xen) begin
	next_state <= current_state;
	case current_state is
	  
		when idle => --idle state
         if fifo_rdempty = '0' then 
            next_state <= smpls_present;
         else 
            next_state <= idle;
         end if;
...
fifo_rdreq <= int_fifo_rdreq AND NOT fifo_rdempty AND int_xen_reg;
...
pulse_div_inst0 : entity work.pulse_div 
port map(
	clk         => clk,
	reset_n     => reset_n,
	pulse_in    => xen,
   pulse_div   => inst0_pulse_div
    );
    
pulse_div_inst1 : entity work.pulse_div 
port map(
	clk         => clk,
	reset_n     => reset_n,
	pulse_in    => inst0_pulse_div,
   pulse_div   => inst1_pulse_div
    );
    
int_xen <= inst0_pulse_div when ch_en_sync = "11" else inst1_pulse_div;
